function [dmin_x, dmax_x, dmin_y, dmax_y] = parcel_distance_limits_with_blocks_in_memory(parc_idx_x, n_parc_x, parc_idx_y, n_parc_y, distance_memory, distance_blocks)
...
%This function computes the minimum positive and maximum within-parcel
%distances for x and y using blockwise traversal of an in-memory full
%distance matrix or strict upper-triangular distance vector.

parc_idx_x = parc_idx_x(:);
parc_idx_y = parc_idx_y(:);

dmin_x_per_block = inf(distance_blocks.n_unique_block_pairs, n_parc_x);
dmax_x_per_block = zeros(distance_blocks.n_unique_block_pairs, n_parc_x);

dmin_y_per_block = inf(distance_blocks.n_unique_block_pairs, n_parc_y);
dmax_y_per_block = zeros(distance_blocks.n_unique_block_pairs, n_parc_y);
...
if distance_blocks.parallel
    parfor (pair_idx = 1:distance_blocks.n_unique_block_pairs, distance_blocks.n_workers)
        ...
        dmin_x_per_block(pair_idx, :) = dmin_x_ij.';
        dmax_x_per_block(pair_idx, :) = dmax_x_ij.';
        dmin_y_per_block(pair_idx, :) = dmin_y_ij.';
        dmax_y_per_block(pair_idx, :) = dmax_y_ij.';
    end
...
dmin_x = min(dmin_x_per_block, [], 1).';
dmax_x = max(dmax_x_per_block, [], 1).';

dmin_y = min(dmin_y_per_block, [], 1).';
dmax_y = max(dmax_y_per_block, [], 1).';
