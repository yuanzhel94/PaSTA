function [vx, vy] = parcel_variogram_with_blocks_in_memory(x, y, parc_idx_x, parc_idx_y, parcel_specs_x, parcel_specs_y, distance_memory, distance_blocks)
%[vx, vy] = parcel_variogram_with_blocks_in_memory(x, y, parc_idx_x, parc_idx_y, parcel_specs_x, parcel_specs_y, distance_memory, distance_blocks)
%This function computes parcel-level variograms for two maps using an
%in-memory distance matrix or strict upper-triangular distance vector in
%blockwise manner
...
if distance_blocks.parallel
    parfor (pair_idx = 1:distance_blocks.n_unique_block_pairs, distance_blocks.n_workers)
        ...
        if ~isempty(dij)
            [weight_sum_x_cell, weighted_diff2_sum_x_cell] = blockij_parcel_variogram_accumulation_single_map(dij, x(idx_i), x(idx_j), parc_idx_x(idx_i), parc_idx_x(idx_j), parcel_specs_x, is_diagonal_block);

            [weight_sum_y_cell, weighted_diff2_sum_y_cell] = blockij_parcel_variogram_accumulation_single_map(dij, y(idx_i), y(idx_j), parc_idx_y(idx_i), parc_idx_y(idx_j), parcel_specs_y, is_diagonal_block);
