function pasta_fit = pasta_memory_inefficient(x,y,coord,varargin)
% This function computes the significance for spatial association using PaSTA
% Inputs:
...
%   crossblock: ways to infer between-parcel covariance. Although 'conv'
    ...
    %   is the default. Only work in PaSTA-NS, i.e., number of parcels are > 1.
...
%   M_scale: the scaling factor to determine the number of lag distances
    ...
    %   settings.
% Outputs:
%   pef: statistical significance p-value derived using PaSTA/PaSTA-NS
%   rX: pearson correlation coefficient
%   nef: effective sample size estiamted with PaSTA/PaSTA-NS
%   run_status: bool indicator whether PaSTA/PaSTA-NS estimation is valid, 1
...
addParameter(p, 'D', [], @(v) isempty(v) || (isnumeric(v) && ismatrix(v) && isreal(v)));
addParameter(p, 'dim', [], @(v) isempty(v) || (isnumeric(v) && isscalar(v) && isreal(v) && isfinite(v) && v >= 1 && fix(v) == v));
...
% now run the stationary stable model without parcellation, i.e., PaSTA
[v1,h1] = estimate_variogram(D, x, M, qd, kernel_scale);
[v2,h2] = estimate_variogram(D, y, M, qd, kernel_scale);
...
% estimate the covariance matrix by parcellation if n_parc > 1, i.e., PaSTA-NS
if xn_parc > 1
    [fc_para1, bs1] = parcellated_cov(x, xparc, xunique_parcs, D, dim, b1, fc_para1, qd, M_scale, nugget, crossblock, kernel_scale);
end
if yn_parc > 1
    [fc_para2, bs2] = parcellated_cov(y, yparc, yunique_parcs, D, dim, b2, fc_para2, qd, M_scale, nugget, crossblock, kernel_scale);
end
...
pasta_fit.n_parc = n_parc;
pasta_fit.p_naive = p_naive;
pasta_fit.fc_para1 = fc_para1;
pasta_fit.fc_para2 = fc_para2;
